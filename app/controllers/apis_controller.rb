    ...
    def login
        ...
        if facility_manager != nil && service != nil
            ...
            if facility != nil && facility.service_id == service.id && facility_manager.authenticate(password)
                jsonMsg(200,"Authenticated",[service.name, service.description, facility.id.to_s, facility.name]) 
        ...
        if facility_manager.authenticate(password)
            ...
            if fridge_id == -1 
                ...
                if fridge.fridge_latest_states.count > 0
                    ...
                    fridge_last.update(current_picture_path: image, current_storage_rate: initial_storage_rate, update_count: update_count, updated_by: facility_manager.identify)
                else
